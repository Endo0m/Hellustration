...
public abstract class EnemyBase : MonoBehaviour
{
    ...
    protected virtual void Update()
    {
        ...
        else if (isChaseTimerActive)
        {
            ...
            {
                // Добавляем явный поиск игрока в агрессивном режиме
                FindAndChasePlayer();
            }
        }
        ...
        // Меняем условие на проверку isChasing
    ...
    protected virtual void EnableAggressiveMode()
    {
        ...
        // Проверяем только на Hidden слой
    ...
    protected void CheckPlayerDetection()
    {
        ...
        foreach (RaycastHit2D hit in frontHits)
        {
            if (hit.collider.CompareTag("Player"))
            {
                ...
                // Проверяем только слой Hidden, в остальных случаях видим игрока
        ...
        // Проверяем, не спрятался ли уже обнаруженный игрок
    ...
    protected void SearchForTeleport(bool searchingUp)
    {
        ...
        foreach (var teleport in teleports)
        {
            ...
            // Телепорт должен вести в том же направлении, куда нам нужно
            if (isTeleportUp == searchingUp)
            {
                // Проверяем, что телепорт действительно приближает нас к игроку
    ...
    protected void ChasePlayer()
    {
        if (playerTransform != null)
        {
            ...
            // Если не используем телепорт и разница по Y слишком большая
